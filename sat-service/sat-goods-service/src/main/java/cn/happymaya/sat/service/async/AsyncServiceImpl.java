package cn.happymaya.sat.service.async;
...
public class AsyncServiceImpl implements IAsyncService {
    ...
    private final SatGoodsDao ecommerceGoodsDao;
    private final StringRedisTemplate redisTemplate;

    public AsyncServiceImpl(SatGoodsDao ecommerceGoodsDao,
    ...
    public void asyncImportGoods(List<GoodsInfo> goodsInfos, String taskId) {
        ...
        List<SatGoods> ecommerceGoods = filteredGoodsInfo.stream()
                .map(SatGoods::to)
                .collect(Collectors.toList());
        List<SatGoods> targetGoods = new ArrayList<>(ecommerceGoods.size());
        ...
        // 商品信息入库
        List<SatGoods> savedGoods = IterableUtils.toList(
     ...
     */
    private void saveNewGoodsInfoToRedis(List<SatGoods> savedGoods) {

        // 由于 Redis 是内存存储, 只存储简单商品信息
        List<SimpleGoodsInfo> simpleGoodsInfos = savedGoods.stream()
                .map(SatGoods::toSimple)
                .collect(Collectors.toList());
