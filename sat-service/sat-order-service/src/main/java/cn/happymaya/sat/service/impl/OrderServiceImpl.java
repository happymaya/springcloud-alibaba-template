...
import cn.happymaya.sat.dao.SatOrderDao;
import cn.happymaya.sat.entity.SatOrder;
import cn.happymaya.sat.feign.AddressClient;
import cn.happymaya.sat.feign.NotSecuredBalanceClient;
import cn.happymaya.sat.feign.NotSecuredGoodsClient;
...
import cn.happymaya.sat.service.IOrderService;
import cn.happymaya.sat.source.LogisticsSource;
...
public class OrderServiceImpl implements IOrderService {
     ...
     * 表的 dao 接口
     */
    private final SatOrderDao orderDao;
    ...
    public OrderServiceImpl(SatOrderDao orderDao, AddressClient addressClient, SecuredGoodsClient securedGoodsClient, NotSecuredGoodsClient notSecuredGoodsClient, NotSecuredBalanceClient notSecuredBalanceClient, LogisticsSource logisticsSource) {
    ...
    public TableId createOrder(OrderInfo orderInfo) {
        ...
        // 获取地址信息
        AddressInfo addressInfo = addressClient.getAddressInfoByTablesId(new TableId(Collections.singletonList(new TableId.Id(orderInfo.getUserAddress())))).getData();
        ...
        // 2. 创建订单
        SatOrder newOrder = orderDao.save(new SatOrder(AccessContext.getLoginUserInfo().getId(), orderInfo.getUserAddress(), JSON.toJSONString(orderInfo.getOrderItems())));
        log.info("create order success: [{}], [{}]", AccessContext.getLoginUserInfo().getId(), newOrder.getId());

        // 3. 扣减商品库存  OrderInfo.OrderItem::toDeductGoodsInventory
        if (!notSecuredGoodsClient.deductGoodsInventory(orderInfo.getOrderItems().stream().map(OrderInfo.OrderItem::toDeductGoodsInventory).collect(Collectors.toList())).getData()) {
            throw new RuntimeException("deduct goods inventory failure");
        }

        // 4. 扣减用户账户余额
        // 4.1 获取商品信息, 计算总价格
        List<SimpleGoodsInfo> goodsInfos = notSecuredGoodsClient.getSimpleGoodsInfoByTableId(new TableId(orderInfo.getOrderItems().stream().map(o -> new TableId.Id(o.getGoodsId())).collect(Collectors.toList()))).getData();
        Map<Long, SimpleGoodsInfo> goodsId2GoodsInfo = goodsInfos.stream().collect(Collectors.toMap(SimpleGoodsInfo::getId, Function.identity()));
        long balance = 0;
        ...
        // 5. 发送订单物流消息 SpringCloud Stream + Kafka
        LogisticsMessage logisticsMessage = new LogisticsMessage(AccessContext.getLoginUserInfo().getId(), newOrder.getId(), orderInfo.getUserAddress(), null    // 没有备注信息
        );
    ...
    public PageSimpleOrderDetail getSimpleOrderDetailByPage(int page) {
        ...
        Page<SatOrder> orderPage = orderDao.findAllByUserId(AccessContext.getLoginUserInfo().getId(), pageable);
        List<SatOrder> orders = orderPage.getContent();
        ...
        orders.forEach(o -> {
            ...
            goodsIdsInOrders.addAll(goodsAndCount.stream().map(DeductGoodsInventory::getGoodsId).collect(Collectors.toSet()));
        });
        ...
        // 获取商品信息
        List<SimpleGoodsInfo> goodsInfos = securedGoodsClient.getSimpleGoodsInfoByTableId(new TableId(goodsIdsInOrders.stream().map(TableId.Id::new).collect(Collectors.toList()))).getData();

        // 获取地址信息
        AddressInfo addressInfo = addressClient.getAddressInfoByTablesId(new TableId(orders.stream().map(o -> new TableId.Id(o.getAddressId())).distinct().collect(Collectors.toList()))).getData();
     ...
     * <h2>组装订单详情</h2>
     */
    private List<PageSimpleOrderDetail.SingleOrderItem> assembleSimpleOrderDetail(List<SatOrder> orders, List<SimpleGoodsInfo> goodsInfos, AddressInfo addressInfo) {
     ...
     * <h2>构造订单中的商品信息</h2>
     */
    private List<PageSimpleOrderDetail.SingleOrderGoodsItem> buildOrderGoodsItem(SatOrder order, Map<Long, SimpleGoodsInfo> id2GoodsInfo) {
