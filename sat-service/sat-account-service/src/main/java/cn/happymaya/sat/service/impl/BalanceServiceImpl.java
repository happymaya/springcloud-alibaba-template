...
import cn.happymaya.sat.dao.SatBalanceDao;
import cn.happymaya.sat.entity.SatBalance;
import cn.happymaya.sat.filter.AccessContext;
import cn.happymaya.sat.service.IBalanceService;
...
public class BalanceServiceImpl implements IBalanceService {
    ...
    private final SatBalanceDao balanceDao;

    public BalanceServiceImpl(SatBalanceDao balanceDao) {
    ...
    public BalanceInfo getCurrentUserBalanceInfo() {
        ...
        SatBalance ecommerceBalance = balanceDao.findByUserId(loginUserInfo.getId());
        if (null != ecommerceBalance) {
            balanceInfo.setBalance(ecommerceBalance.getBalance());
        } else {
            // 如果还没有用户余额记录, 这里创建出来，余额设定为0即可
            SatBalance newBalance = new SatBalance();
