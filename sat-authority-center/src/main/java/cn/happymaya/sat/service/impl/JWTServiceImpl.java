...
import cn.happymaya.sat.dao.EcommerceUserDao;
...
public class JWTServiceImpl implements IJWTService {
    ...
    public String generateToken(String username, String password, int expire)
        ...
        SatUser satUser = ecommerceUserDao.findByUsernameAndPassword(
                username, password
        );
        if (null == satUser) {
        ...
        LoginUserInfo loginUserInfo = new LoginUserInfo(
                satUser.getId(), satUser.getUsername()
        );
    ...
    public String registerUserAndGenerateToken(UsernameAndPassword usernameAndPassword)
        ...
        SatUser oldUser = ecommerceUserDao.findByUsername(
        ...
        // 注册一个新用户, 写一条记录到数据表中
        satUser = ecommerceUserDao.save(satUser);
        log.info("register user success: [{}], [{}]", satUser.getUsername(),
                satUser.getId());
